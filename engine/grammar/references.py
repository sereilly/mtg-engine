...
# The union of object references one verb reaches — "Destroy target creature **and
# target land**", "**Each attacking creature and each blocking creature**
# doesn't untap …". Here rather than in one of the effect families because four
# modules read it — `board`, `destruction`, `imperatives` and `subject_verb` —
# and a fragment two families need is not one family's property. It lived in
# `effects/board.py` until `destruction` split out of that module and needed it
# too, which is the coupling the shared-module rule is about.
#
# Here rather than in `phrases` one layer up, and the reason is what it reads:
# `parse_recipient` and `parse_bound_subject` are both defined in this module,
# and a union of them is a question about object *references* rather than a
# word table. Every effects family already reaches this layer for the singular
# form, so the plural costs no new import.
...
def _parse_further_subjects(
    ...
    *,
    ...
    while True:
        ...
        # A bound "that <type>" is read by `parse_bound_subject` and by nothing
        # else — deliberately, because the phrase turns up all over the pool and
        # the shared noun parser would hand it to every line printing it. So the
        # union asks for it in the same order the subject position does, and
        # only where it is safe to: in front of the verb.
        if nxt is None and separated and before_verb and not stream.at_word("to"):
            nxt = parse_bound_subject(stream)
        ...
        # **"that <type>" is the fifth, and only in front of the verb.**
        # "This creature **and that creature** phase out" (Dream Fighter) is a
        # union of the source and the object its trigger bound. In the object
        # position the same words are almost always a new clause ("destroy it
        # **and that creature** gains…"), which is the hazard the whole
        # quantifier gate is written for — but *before the verb* the caller has
        # read a subject and no verb yet, so an "and" here cannot be joining two
        # clauses. That is the caller's own stated reason for probing at all,
        # and this flag is it said out loud.
        allowed = ("all", "each", "target", "this")
        if before_verb:
            allowed += ("that",)
        if not isinstance(nxt, ast.TargetSpec) or nxt.quantifier not in allowed:
            stream.reset(mark)
            return extra
        ...
        # …and the end-of-sentence test is exactly that hazard, so it is asked
