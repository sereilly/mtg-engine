...
``_accept_life_alternative`` travels with them, and it is the one name here two
other modules reach for (``sentence_clauses``, ``subject_verb``). That is not a
family coupling: they reach it through ``effects``' flat re-export, which is the
address every production in this package has, and the fragment is the life half
of exactly this offer.
"""
