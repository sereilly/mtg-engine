"""Tapping and untapping (CR 701.20, 701.26) — the productions, one family.

Split out of `effects/board.py` at the thousand-line guard, reusing the name
`lowering/tapping.py` has carried since it left `lowering/board.py`: the mirror
re-forms rather than forking, which is what the family names are for.

The boundary is the CR's own. Everything here is about whether a permanent is
*tapped* — the keyword actions, the untap-step restrictions that suspend one,
and the toll a player may pay to undo one — where the rest of `board.py`
destroys, bounces, sacrifices or re-controls. The two halves shared no helper:
what they both need (`_parse_that_object`, the mana payment, the noun phrases)
already lives one layer down in `phrases`, which is where a fragment two
families need belongs.
"""
...
from ..phrases import _parse_mana_payment, parse_bound_subject, parse_subject_filter_at
