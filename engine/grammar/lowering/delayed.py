"""Lowering delayed triggered abilities (CR 603.7), and the two-sentence
spells that hand one its subject.

"**At the beginning of the next end step**, sacrifice it" — an ability created
by a resolution that fires later, which is a different lowering problem from
every other statement here: the effect has to be frozen now and performed
against a board nobody can see yet, so what the delay may *refer to* is a closed
set (``DELAYED_EVENTS``) and what it may carry forward is whatever an earlier
step of the same effect recorded.

Split out of ``lowering/stack.py`` when that module crossed the thousand-line
guard, along the line its own section header already drew. The two halves are
both about an object that is not on the battlefield, which is why they shared a
module — but a counterspell acts on something **on the stack right now** and a
delayed ability acts on something that will exist **later**, and only this half
grows with the pool: every set prints more delayed riders and the counter
templates are largely closed. That is the split the playbook asks for — cut
where the docstring already draws a line, and keep the half that grows.

``_lower_choose_target`` and ``_lower_waive_shroud`` come with it rather than
staying behind: both are the *first* sentence of a two-sentence spell whose
second sentence is the delayed half, so they are read together or not at all.
"""
