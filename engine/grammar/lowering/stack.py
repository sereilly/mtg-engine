"""Lowering the sentences about an object waiting on the stack.

Countering a spell or an ability, copying one, retargeting one, and the
modal head every modal spell is announced with.
...
def _lower_counter_spell(node: ast.CounterSpell) -> tuple[OracleInstruction, ...]:
    ...
    if node.unless_pays is not None:
        if node.unless_pays_life:
            # "…pays {1} **and 1 life**." (Mundungu.) One offer with two
            # prices: it rides the same payment payload rather than a second
            # instruction, because two prompts would be two decisions and two
            # counters. CR 119.4 caps it at the payer's life total, which the
            # payment prompt asks — not this lowering, which knows no board.
            payload["unless_pays_life"] = int(node.unless_pays_life)
...
def _lower_counter_ability(
    ...
    if node.unless_pays is not None:
        ...
        if node.unless_pays_life:
            # The life half of the same offer, one object over — see the spell
            # counter's branch above. Both arm the one ``mana_payment`` prompt,
            # so this is the whole of what the ability side needs.
            payload["unless_pays_life"] = int(node.unless_pays_life)
