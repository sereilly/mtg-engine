...
Split out of ``damage`` the round Tracker's second sentence pushed that module
past the thousand-line guard, and along the line the CR already draws: a
redirection is a **replacement effect** (CR 614.9), not a damage effect. The
damage is still dealt, in full, by the same source, and only its recipient
changes — which is why ``ast/damage.py`` gives it a node of its own and says
that a redirect read as a shield loses lifelink, the damage triggers and the
dealt-damage records all at once.

The parse side keeps redirection with damage because the two read the same
recipient, source and duration vocabulary; the lowering halves share nothing
but the three generic helpers every family here uses. That is the same
asymmetry ``prevention`` records one module over — a lowering half that
outgrew its parse half.
"""
