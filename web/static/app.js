...
function getPayLifeToSaveInfo(state = currentState) {
  if (!state || seat === null) return null;
  const info = state.pay_life_to_save;
  if (!info) return null;
...
// Truce: "each player may draw up to two cards" — the seat says how many.
// Its own prompt rather than the number choice below it because the answer is
// performed here (the cards are drawn as it is answered) and the range comes
// from the card's ceiling rather than from a permanent.
function getDrawUpToInfo(state = currentState) {
  if (!state || seat === null) return null;
  const info = state.draw_up_to;
...
function getNumberChoiceInfo(state = currentState) {
  if (!state || seat === null) return null;
  const info = state.number_choice;
...
// contributes - a list of names alone could not tell whether a selection pays.
let aggregateSacrificeSelected = [];

function getAggregateSacrificeInfo(state = currentState) {
  if (!state || seat === null) return null;
  const info = state.aggregate_sacrifice;
  if (!info || info.player_seat !== seat) return null;
  if (!Array.isArray(info.choices)) return null;
  return info;
}

function applyAggregateSacrificePrompt(info) {
  ...
  const offered = new Map(info.choices.map((c) => [c.permanent_id, c]));
  aggregateSacrificeSelected = aggregateSacrificeSelected.filter((id) => offered.has(id));
  const wanted = Number(info.at_least) || 0;
  const total = aggregateSacrificeSelected.reduce(
    (sum, id) => sum + Number(offered.get(id).amount || 0), 0,
  );

  title.textContent = `${info.card_name} — choose what to sacrifice`;
  body.textContent =
    `Sacrifice any number of them with total ${info.characteristic} ${wanted} or greater.`;
  const buttons = info.choices
    .map(
      (c) =>
        `<button type="button" class="prompt-choice-btn` +
        `${aggregateSacrificeSelected.includes(c.permanent_id) ? " selected" : ""}"` +
        ` data-agg-id="${c.permanent_id}">` +
        `${escapeHtml(c.name)} (${c.amount})</button>`
    )
    .join("");
  steps.innerHTML =
    `<div class="prompt-choice-column">${buttons}</div>` +
    `<div class="prompt-choice-row"><button type="button" class="prompt-choice-btn"` +
    `${total >= wanted ? "" : " disabled"} data-agg-confirm="1">` +
    `Confirm (${total}/${wanted})</button></div>`;

  steps.querySelectorAll("[data-agg-id]").forEach((btn) => {
    btn.addEventListener("click", () => {
      const id = Number(btn.dataset.aggId);
      aggregateSacrificeSelected = aggregateSacrificeSelected.includes(id)
        ? aggregateSacrificeSelected.filter((other) => other !== id)
        : [...aggregateSacrificeSelected, id];
      applyAggregateSacrificePrompt(info);
    });
  });
  steps.querySelector("[data-agg-confirm]")?.addEventListener("click", async () => {
    if (total < wanted) return;
    const picks = [...aggregateSacrificeSelected];
    aggregateSacrificeSelected = [];
    await sendAction({
      seat, action: "aggregate_sacrifice_confirm", target_permanent_ids: picks,
    });
  });
}

// Ether Well: "If that creature is red, you may put it on the bottom of its
// owner's library instead." The permanent is still on the battlefield while
// this is owed - the move is what the answer performs.
function getLibraryEndChoiceInfo(state = currentState) {
  if (!state || seat === null) return null;
  const info = state.library_end_choice;
  if (!info || info.player_seat !== seat) return null;
  return info;
}
...
// Deflection: "Change the target of target spell with a single target." The
// candidates are heterogeneous -- whatever that spell could legally have
// chosen -- so the label says which kind each one is.
function getRetargetChoiceInfo(state = currentState) {
  if (!state || seat === null) return null;
  const info = state.retarget_choice;
  if (!info || !Array.isArray(info.options) || info.options.length === 0) return null;
  return info;
}
...
// Forbidden Ritual: whether the spell's controller runs the printed process
// again. No stake and no count — "any number of times" names neither.
function getRepeatProcessInfo(state = currentState) {
  if (!state || seat === null) return null;
  const info = state.repeat_process;
  if (!info || info.player_seat !== seat) return null;
  return info;
}
...
function getExileFromHandInfo(state = currentState) {
  if (!state || seat === null) return null;
  const info = state.exile_from_hand_choice;
  if (!info || !Array.isArray(info.choices) || info.choices.length === 0) return null;
  return info;
}
...
// One land at a time, offered to every seat in turn, so the prompt names the
// permanent on the table right now.
function applyPayLifeToSavePrompt(info) {
  const panel = q("activationPanel");
  const title = q("promptTitle");
  const body = q("promptBody");
  const steps = q("promptSteps");
  ...
  const life = Number(info.life || 1);
  const permanentName = info.permanent_name || "that permanent";
  title.textContent = "Pay Life to Save?";
  body.textContent =
    `${info.card_name || "An effect"} will destroy ${permanentName} ` +
    `unless a player pays ${life} life.`;
  steps.innerHTML = [
    `<div>Permanent: ${escapeHtml(permanentName)}</div>`,
    `<div class="prompt-choice-row">` +
      `<button type="button" class="prompt-choice-btn" data-pay-life-save="yes">Pay ${life} life</button>` +
      `<button type="button" class="prompt-choice-btn" data-pay-life-save="no">Let it be destroyed</button>` +
      `</div>`,
  ].join("");

  steps.querySelectorAll("[data-pay-life-save]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      await sendAction({
        seat,
...
// the buttons come from the server rather than being counted out here.
function applyNumberChoicePrompt(info) {
  const panel = q("activationPanel");
  const title = q("promptTitle");
  const body = q("promptBody");
  const steps = q("promptSteps");
  ...
  const cardName = info.card_name || "";
  title.textContent = "Choose a number";
  body.textContent = `${cardName}: choose a number between ${info.minimum} and ${info.maximum}.`;
  const buttons = info.options
    .map(
      (n) =>
        `<button type="button" class="prompt-choice-btn" data-number="${escapeHtml(String(n))}">` +
        `${escapeHtml(String(n))}</button>`
    )
    .join("");
  steps.innerHTML = `<div class="prompt-choice-column">${buttons}</div>`;

  steps.querySelectorAll("[data-number]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      await sendAction({
        seat,
...
// Truce: "Each player may draw up to two cards." One button per legal answer,
// zero included — declining is what the card is for.
function applyDrawUpToPrompt(info) {
  const panel = q("activationPanel");
  const title = q("promptTitle");
  const body = q("promptBody");
  const steps = q("promptSteps");
  ...
  const cardName = info.card_name || "";
  title.textContent = "Draw how many?";
  body.textContent =
    `${cardName}: you may draw up to ${info.maximum} card(s). ` +
    `Your library holds ${info.library_size}.`;
  const buttons = info.options
    .map(
      (n) =>
        `<button type="button" class="prompt-choice-btn" data-draw-count="${escapeHtml(String(n))}">` +
        `${escapeHtml(String(n))}</button>`
    )
    .join("");
  steps.innerHTML = `<div class="prompt-choice-column">${buttons}</div>`;

  steps.querySelectorAll("[data-draw-count]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      await sendAction({
        seat,
...
function applyLibraryEndPrompt(info) {
  // `index` is the field the shared button prompt reads for anything that is
  // not a seat, and 0/1 is what `to_bottom` means on the wire - top first,
  // because that is the end the card puts the creature on by default.
  applyChoiceButtonPrompt(
    {
      ...info,
      prompt: `Put ${info.moved_name} on which end of ${info.owner_name}'s library?`,
      options: [{ index: 0, label: "Top" }, { index: 1, label: "Bottom" }],
    },
    {
      title: "Choose an end of the library",
...
function applyRetargetChoicePrompt(info) {
  applyChoiceButtonPrompt(info, {
    title: "Choose a new target",
    action: "retarget_choice_confirm",
    field: "target_index",
    // A face and a permanent can share a name in this game (a player called
    // after their commander), so the kind is on the button rather than left to
    // be inferred.
    labelOf: (option) =>
      option.kind === "player" ? `${option.name} (player)` : option.name,
  });
}
...
function applyRepeatProcessPrompt(info) {
  const panel = q("activationPanel");
  const title = q("promptTitle");
  const body = q("promptBody");
  const steps = q("promptSteps");
  ...
  title.textContent = `${info.card_name} \u2014 repeat?`;
  body.textContent = "You may repeat this process any number of times.";
  steps.innerHTML =
    '<div class="prompt-choice-column">' +
    '<button type="button" class="prompt-choice-btn" data-repeat-again="1">Repeat</button>' +
    '<button type="button" class="prompt-choice-btn" data-repeat-stop="1">Stop</button>' +
    "</div>";

  steps.querySelector("[data-repeat-again]")?.addEventListener("click", async () => {
    await sendAction({ seat, action: "repeat_process_confirm", accept: true });
  });
  steps.querySelector("[data-repeat-stop]")?.addEventListener("click", async () => {
    await sendAction({ seat, action: "repeat_process_confirm", accept: false });
  });
}
...
function applyExileFromHandPrompt(info) {
  ...
  const buttons = info.choices
    .map(
      (c) =>
        `<button type="button" class="prompt-choice-btn" data-efh-hand="${c.hand_index}">` +
        `${escapeHtml(c.name)}</button>`
    )
    .join("");
  steps.innerHTML =
  ...
  steps.querySelectorAll("[data-efh-hand]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      await sendAction({
        ...
        hand_index: Number(btn.dataset.efhHand),
      });
    });
  });
...
function renderActivationPrompt() {
  ...
  const aggregateSacrificeInfo = getAggregateSacrificeInfo();
  if (aggregateSacrificeInfo) {
    applyAggregateSacrificePrompt(aggregateSacrificeInfo);
    return;
  }

  const libraryEndInfo = getLibraryEndChoiceInfo();
  if (libraryEndInfo) {
    applyLibraryEndPrompt(libraryEndInfo);
    return;
  }
  ...
  const repeatProcessInfo = getRepeatProcessInfo();
  if (repeatProcessInfo) {
    applyRepeatProcessPrompt(repeatProcessInfo);
    return;
  }
