...
- Not Implemented: no ``cards/<CODE>_cards.json`` file exists.
- Partial: a card file exists but not every card in the set is present
  and supported by the oracle compiler.
...
def implementation_state(set_entry: dict) -> tuple[str, str]:
    """Return (state, detail) for one set based on local card data."""
    card_file = CARDS_DIR / f"{set_entry['code'].upper()}_cards.json"
...
def render_markdown(sets: list[dict], fetched_at: str) -> str:
    ...
    lines = [
        ...
        "",
        f"**Progress: {complete} complete, {partial} partial, "
        f"{len(sets) - complete - partial} not implemented ({len(sets)} sets)**",
        "",
