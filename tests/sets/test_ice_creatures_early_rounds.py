"""Ice Age (ICE) creature cards — the serial rounds (1–41).
...
This file is the *first* half of that split's second axis. ICE's creatures
passed the 2,600-line guard, and tests/sets/README.md's next division past
the printed type is a **round boundary**, each section being self-contained
and written up under its own round in ROADMAP.md. The cut is where the set
stopped being implemented one round at a time: rounds 1–41 are here, and the
parallel wave that finished the set is in ``test_ice_creatures.py``.
